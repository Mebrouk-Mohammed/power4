...
func Init() error {
	// DB SQLite stockée par défaut dans ./auth/users.db
	// On accepte un emplacement personnalisé via POWER4_AUTH_DB
	dbPath := filepath.Join("auth", "users.db")
	if env := os.Getenv("POWER4_AUTH_DB"); env != "" {
		dbPath = env
	}
	...
	tpl, err = template.ParseFiles(
		filepath.Join("templates", "connexion", "login.gohtml"),
		filepath.Join("templates", "connexion", "register.gohtml"),
		filepath.Join("templates", "connexion", "home.gohtml"),
	)
	return err
}
